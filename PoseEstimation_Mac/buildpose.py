...
class CameraCalibration:
    ...
    def update_from_calibration(self, objpoints, imgpoints_left, imgpoints_right, img_shape):
        """Recompute stereo calibration and update P matrices live."""
        ret0, mtx0, dist0, rvecs0, tvecs0 = cv2.calibrateCamera(objpoints, imgpoints_left, img_shape, None, None)
        ret1, mtx1, dist1, rvecs1, tvecs1 = cv2.calibrateCamera(objpoints, imgpoints_right, img_shape, None, None)
...
def main():
    ...
    pairs_captured = 0
    ret0_cb = ret1_cb = False
    gray0 = None
    gray1 = None
    corners0 = corners1 = None
    is_fullscreen = False
    ...
    while True:
        ...
        # ── Branch based on Calibration Mode ──
        if calibration_mode and len(streams) >= 2:
            frame0 = frames[0]
            frame1 = frames[1]
            ...
            draw_info_overlay(disp0, f"{streams[0].name} (Calib - Pairs: {pairs_captured})")
            draw_info_overlay(disp1, f"{streams[1].name} ({'Ready!' if ret0_cb and ret1_cb else 'Searching...'})")
            
            frame_outputs.append(disp0)
            frame_outputs.append(disp1)
            
            for i in range(2, len(frames)):
                f_copy = frames[i].copy()
                draw_info_overlay(f_copy, f"{streams[i].name} (Idle)")
                frame_outputs.append(f_copy)
                
            empty = np.zeros((FRAME_H, FRAME_W, 3), dtype=np.uint8)
            cv2.putText(empty, "CALIBRATION MODE", (50, 180), FONT, 1.0, COL_ACCENT2, 2)
            cv2.putText(empty, f"Pairs captured: {pairs_captured}", (50, 220), FONT, 0.8, COL_WHITE, 2)
            cv2.putText(empty, "[SPACE] Capture pair", (50, 260), FONT_SMALL, 0.6, COL_ACCENT, 1)
            cv2.putText(empty, "[C] Compute matrix", (50, 290), FONT_SMALL, 0.6, COL_ACCENT, 1)
            cv2.putText(empty, "[B] Exit Calib Mode", (50, 320), FONT_SMALL, 0.6, COL_GRAY, 1)
            frame_outputs.append(empty)
        ...
        if key == ord("q") or key == 27:
            ...
            if calibration_mode:
                print("Entered Calibration Mode. YOLO paused.")
            else:
                print("Exited Calibration Mode. YOLO resumed.")
        elif calibration_mode and key == 32: # SPACE
            if len(streams) >= 2 and ret0_cb and ret1_cb:
                ...
                calib_objpoints.append(objp)
                calib_imgpoints_left.append(corners0_sub)
                calib_imgpoints_right.append(corners1_sub)
                pairs_captured += 1
            ...
            if pairs_captured < 5:
                ...
                try:
                    rms = calib.update_from_calibration(calib_objpoints, calib_imgpoints_left, calib_imgpoints_right, gray0.shape[::-1])
                    print(f"Calibration updated! Stereo RMS: {rms:.3f}")
                    calibration_mode = False
                    calib_objpoints.clear()
                    calib_imgpoints_left.clear()
                    calib_imgpoints_right.clear()
                    pairs_captured = 0
        ...
        elif not calibration_mode and key == ord("c"):
