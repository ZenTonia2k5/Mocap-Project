...
def main():
    parser = argparse.ArgumentParser(description="MediaPipe Pose Estimation and Validation with FPS and Metrics Logging")
    ...
    parser.add_argument("--conf", type=float, default=0.5,
                        help="Minimum detection/tracking confidence threshold")
    ...
    print(f"--- Running on: {device_name} (MediaPipe processes on CPU/XNNPACK) ---")
    ...
    model_info = model_map[args.complexity]
    ...
    if os.path.exists(model_name):
        ...
        if not os.path.exists(model_filepath):
            print(f"Model file '{model_filepath}' not found. Downloading from {model_info['url']}...")
            urllib.request.urlretrieve(model_info["url"], model_filepath)
            print("Download complete.")

    print(f"Loading MediaPipe Pose Landmarker: {model_filepath}...")
    base_options = python.BaseOptions(model_asset_path=model_filepath)
    ...
    if args.mode == "val":
        ...
        while True:
            ...
            val_out_dir = os.path.join(val_out_base, f"mediapipe_val{suffix}")
        ...
        for img_idx, img_file in enumerate(image_files):
            ...
            if results.pose_landmarks:
                for pose_landmarks in results.pose_landmarks:
                    pred_kpts = []
                    for idx in coco_mapping:
                        landmark = pose_landmarks[idx]
                        pred_kpts.append([landmark.x, landmark.y, landmark.visibility])
                    pred_kpts = np.array(pred_kpts)
                    
                    pred_poses.append({
                        "keypoints": pred_kpts,
                        "conf": np.mean(pred_kpts[:, 2]) 
                    })
        ...
        validation_summary = {
            ...
            "Total Validation Time (s)": f"{time.time() - start_val_time:.2f}"
        }
        
        log_to_file(args.log, f"mediapipe-complexity-{args.complexity}", "Validation", validation_summary, device_name, cuda_available)
        detector.close()
        return
        ...
        print(f"--- Running MediaPipe Prediction on Source: {source_val} ---")
        ...
        if not args.no_show:
            cv2.namedWindow('MediaPipe Pose Stream', cv2.WINDOW_NORMAL)
            cv2.resizeWindow('MediaPipe Pose Stream', 1280, 720)
        ...
        while cap.isOpened():
            ...
            if results.pose_landmarks:
                detections_count += len(results.pose_landmarks)
                from mediapipe.framework.formats import landmark_pb2
                for pose_landmarks in results.pose_landmarks:
                    pose_landmarks_proto = landmark_pb2.NormalizedLandmarkList()
                    pose_landmarks_proto.landmark.extend([
                    ...
                    for landmark in pose_landmarks:
                        all_confs.append(landmark.visibility)
            
            cv2.putText(annotated_frame, f"FPS: {fps:.1f}", (20, 50), 
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)

            if not args.no_show:
                cv2.imshow('MediaPipe Pose Stream', annotated_frame)
                ...
                if key == ord('q'):
                    ...
                    capture_path = os.path.join(capture_dir, f"mediapipe_capture_{timestamp_str}.jpg")
        ...
        log_to_file(args.log, f"mediapipe-complexity-{args.complexity}", "Prediction", prediction_summary, device_name, cuda_available)
